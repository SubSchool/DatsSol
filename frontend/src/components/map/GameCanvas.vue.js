...
function toWorldCoordinate(screenX, screenY) {
    if (!stage || !props.world)
        return null;
...
function clampWorld() {
    if (!host.value || !stage || !props.world)
        return;
...
function fitWorld(force = false) {
    if (!host.value || !stage || !props.world)
        return;
    if (hasInitialFit && !force)
        return;
    const width = boardWidth();
    const height = boardHeight();
    const scale = clamp(Math.min(host.value.clientWidth / width, host.value.clientHeight / height), 0.22, 1.6);
    stage.scale.set(scale);
    stage.position.set((host.value.clientWidth - width * scale) / 2, (host.value.clientHeight - height * scale) / 2);
    clampWorld();
    hasInitialFit = true;
}
function centerOnPosition(position, preferredScale) {
    if (!host.value || !stage)
        return;
    ...
    stage.position.set(host.value.clientWidth / 2 - center.x * nextScale, host.value.clientHeight / 2 - center.y * nextScale);
    clampWorld();
}
...
function createLabel(text, x, y, size = 12, tint = '#f7f0d8') {
...
function drawBoardBackdrop(layer) {
    if (!props.world)
        return;
...
function drawBoostedCells(layer) {
    if (!props.world)
        return;
...
function drawForecasts(layer) {
    ...
    props.world.forecasts.forEach((forecast) => {
        ...
        const label = createLabel(`${forecast.id ?? 'storm'} · ${forecast.forming ? 'forming' : 'moving'}`, center.x - radius, center.y - radius - 20, 11, '#ffd79f');
...
function actionColor(action) {
    ...
    if (action.kind === 'sabotage')
        return 0xff6b6b;
    if (action.kind === 'beaver_attack')
        return 0xffc86b;
    return 0x9af7c3;
}
function drawPlans(layer) {
    if (!props.world)
        return;
...
function drawBackgroundInteractionLayer(layer) {
    if (!props.world)
        return;
...
function drawConstructions(layer) {
    if (!props.world)
        return;
...
function drawPlantations(layer) {
    if (!props.world)
        return;
...
function onPointerMove(event) {
    if (!stage || !dragging)
        return;
    stage.position.set(layerOrigin.x + (event.clientX - dragOrigin.x), layerOrigin.y + (event.clientY - dragOrigin.y));
    clampWorld();
}
...
watch(() => `${props.commandMode}-${props.manualActionKind}-${props.selectedPlantationIds.join(',')}`, () => {
    renderWorld();
});
onBeforeUnmount(() => {
...
(__VLS_ctx.world?.width ?? 0);
(__VLS_ctx.world?.height ?? 0);
__VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
    ...{ class: "overlay-pill" },
});
__VLS_asFunctionalElement(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({
...
(__VLS_ctx.commandMode ? 'command' : 'browse');
__VLS_asFunctionalElement(__VLS_intrinsicElements.div, __VLS_intrinsicElements.div)({
    ...{ class: "overlay-pill" },
});
__VLS_asFunctionalElement(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({
...
__VLS_asFunctionalElement(__VLS_intrinsicElements.span)({
    ...{ class: "legend-swatch beaver" },
});
__VLS_asFunctionalElement(__VLS_intrinsicElements.span, __VLS_intrinsicElements.span)({});
/** @type {__VLS_StyleScopedClasses['canvas-shell']} */ ;
/** @type {__VLS_StyleScopedClasses['canvas-host']} */ ;
/** @type {__VLS_StyleScopedClasses['canvas-overlay']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-pill']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-label']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-value']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-pill']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-label']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-value']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-pill']} */ ;
...
/** @type {__VLS_StyleScopedClasses['overlay-label']} */ ;
/** @type {__VLS_StyleScopedClasses['overlay-value']} */ ;
/** @type {__VLS_StyleScopedClasses['canvas-legend']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-row']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-swatch']} */ ;
/** @type {__VLS_StyleScopedClasses['boosted']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-row']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-swatch']} */ ;
/** @type {__VLS_StyleScopedClasses['allied']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-row']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-swatch']} */ ;
/** @type {__VLS_StyleScopedClasses['enemy']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-row']} */ ;
/** @type {__VLS_StyleScopedClasses['legend-swatch']} */ ;
/** @type {__VLS_StyleScopedClasses['beaver']} */ ;
var __VLS_dollars;
